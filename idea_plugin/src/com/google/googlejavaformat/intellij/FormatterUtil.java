    ...
    static Map<TextRange, String> getReplacements(String text, Collection<TextRange> ranges) {
        try {
            ...
            String jarPath = String.format(
                "%s/devops/format/yext-java-formatter.jar",
                System.getenv("ALPHA"));
            File formatterJar = new File(jarPath);
            ClassLoader classLoader = URLClassLoader.newInstance(
                new URL[]{
                    formatterJar.toURI().toURL()
                });

            Class<?> formatterClass = Class.forName("com.google.googlejavaformat.java.Formatter", true, classLoader);
            Class<?> replacementClass = Class.forName("com.google.googlejavaformat.java.Replacement", true, classLoader);
            ...
            Method getFormatReplacements = formatterClass.getMethod("getFormatReplacements", String.class, Collection.class);
            Object replacementsList = getFormatReplacements.invoke(formatter, text, toRanges(ranges, classLoader));
        ...
        } catch (InvocationTargetException | MalformedURLException | ClassNotFoundException | IllegalAccessException | InstantiationException | NoSuchMethodException e) {
            return ImmutableMap.of();
        }
    }

    private static Collection<Object> toRanges(
